 ...
 * @version $Revision$
 */
public class RpropNeuralNode
  extends RpropNeuralConnection {
  ...
  /** The change in the weights. this is delta in Rprop algorithm . weight update */
  ...
  private double[] m_deltaUpdate;
  
  //###########################################################

  /** Performs the operations for this node. Currently this
   * defines that the node is either a sigmoid or a linear unit. */
  private RpropNeuralMethod m_methods;


  //#########################################################################
  // tracking previous error rate  
  private double[]  m_previous_error;
  
  //avoid overflow problems of floating point variables(max value of weight update)
  private double m_deltaMax;
  
  ////avoid underflow problems of floating point variables(min value of weight update)
   ...
   */
  public RpropNeuralNode(String id, Random r, RpropNeuralMethod m) {
    super(id);
    m_weights = new double[1];
    m_bestWeights = new double[1];
    m_changeInWeights = new double[1];
    m_previous_error=new double[1];
    m_deltaUpdate=new double[1];
   ...
   */
  public void setMethod(RpropNeuralMethod m) {
    m_methods = m;
  } 

  public RpropNeuralMethod getMethod() {
    return m_methods;
  }
  ...
  protected void allocateInputs() {
    ...
    double[] temp7=new double[m_previous_error.length + 15];
    double[] temp8=new double[m_deltaUpdate.length + 15];

    temp4[0] = m_weights[0];
    temp5[0] = m_changeInWeights[0];
    temp6[0] = m_bestWeights[0];
    temp7[0] = m_previous_error[0];
    temp8[0] = m_deltaUpdate[0];
    for (int noa = 0; noa < m_numInputs; noa++) {
      ...
      temp7[noa+1] = m_previous_error[noa+1];
      temp8[noa+1] = m_deltaUpdate[noa+1];
    }
